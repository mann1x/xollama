...
func ggufMetadataPath(digest string) (string, error) {
	...
	return filepath.Join(ggufMetadataDir(), strings.ReplaceAll(digest, ":", "-")+".json"), nil
}

func ggufMetadataDir() string {
	return filepath.Join(envconfig.Models(), "metadata")
}
...
func readGGUFMetadata(digest string) (ggufMetadata, error) {
	...
	writeGGUFMetadata(path, blob, md)
	return md, nil
}
...
func writeGGUFMetadata(path, blob string, md ggufMetadata) {
	...
	if err == nil {
		// Blob removal precedes metadata removal. Rechecking after publication
		// ensures either this writer or a concurrent remover cleans up the file.
		if _, statErr := os.Stat(blob); errors.Is(statErr, os.ErrNotExist) {
			if removeErr := os.Remove(path); removeErr != nil && !errors.Is(removeErr, os.ErrNotExist) {
				slog.Debug("could not remove gguf metadata file", "path", path, "error", removeErr)
			}
			return
		}
	}
...
func pruneGGUFMetadata() {
	...
	if err != nil {
		slog.Debug("could not read gguf metadata dir", "path", dir, "error", err)
		return
	}
	...
	for _, entry := range entries {
		...
		if _, err := os.Stat(blob); err == nil || !errors.Is(err, os.ErrNotExist) {
			continue
		}
