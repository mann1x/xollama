...
func TestOllamaServeArgs(t *testing.T) {
	tests := []struct {
		...
		{
			name: "system xollama serve",
			args: []string{"xollama", "serve"},
			want: true,
		},
		{
			name: "relative path xollama serve",
			args: []string{"./xollama", "serve"},
			want: true,
		},
		{
			name: "serve after other flags",
			args: []string{"./xollama", "--verbose", "serve"},
			want: true,
		},
		{
			name: "start alias",
			args: []string{"xollama", "start"},
			want: true,
		},
		{
			name: "launch command",
			args: []string{"xollama", "launch", "opencode"},
			want: false,
		},
		{
			name: "run command with model named serve",
			args: []string{"xollama", "run", "serve"},
			want: false,
		},
		{
			name: "launch command with serve in passthrough args",
			args: []string{"xollama", "launch", "codex", "--", "-p", "serve"},
			want: false,
		},
		{
		...
		{
			// A stock ollama alongside xollama is an explicitly supported
			// setup, so its serve process must not be claimed as ours.
			name: "stock ollama serve is not ours",
